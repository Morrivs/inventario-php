<?php
 ...
 producto.producto_stock, producto.producto_foto, categoria.categoria_nombre, usuario.usuario_nombre, 
 ...
 if(isset($busqueda) && $busqueda!=""){
     ...
     WHERE producto.producto_codigo LIKE '%$busqueda%' OR
     producto.producto_nombre LIKE '%$busqueda%'
     ORDER BY producto.producto_nombre ASC
     LIMIT $inicio, $registros";

     $consulta_total = "SELECT COUNT(producto_id) FROM producto WHERE producto.producto_codigo LIKE '%$busqueda%' OR
     producto.producto_nombre LIKE '%$busqueda%'"; 
    ...
    $consulta_total = "SELECT COUNT(producto_id) FROM producto WHERE producto.categoria_id = '$categoria_id'"; 
 ...
 $datos=$datos->fetchAll();//pone todos los datos de la tabla en un array

 $total=$conexion->query($consulta_total);
 $total=(int) $total->fetchColumn();

 $Npaginas=ceil($total/$registros);//calcula el numero de paginas que habra en el listado

 
 if ($total>=1 && $pagina<=$Npaginas) {
 //pagina que si existe
 $contador=$inicio+1;
 $pag_inicio=$inicio+1;

//substr() limita la cantidad de carateres
 ...
 $pag_final=$contador-1;
     ...
     if ($total>=1) {
         //cuando si hay registros en el sistema
         $tabla.='
         ...
         //cuando no hay registros presentara eso
